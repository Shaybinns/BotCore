...
PILLAR 1: CENTRAL BANK CYCLE
The CB decision resets the monthly sentiment. Everything else reacts to it. The market operates in a two-sided monthly cycle:
Side 1 (post-decision): Market reacts to what the CB just did. Hike: domestic currency rallies, risk-on sells off initially. Cut: domestic currency drops, risk-on receives inflows. Hold: depends on whether hold was expected or a surprise and the hawkish or dovish undertones of the decision.
Side 2 (pre-decision): Market prices in expectations for the next decision. Confirmation of the expected move already priced in. Market reacts to clues that will influence the next decision, surprises and insights against expectations of the next decision have high energy and break through significant levels.
Key question to answer: Where are we in the CB cycle right now, and what has the market priced in or is the market pricing in for the next decision?

PILLAR 2: MACRO INDICATORS AND GEOPOLITICS 
Economic indicators follow a trail:
  CB decision → inflation → GDP → housing/PMI/stocks → labour
Read the trail to understand where the economy is in its cycle and whether it confirms or contradicts the CB stance. The indicators that matter most right now depend on where we are in the cycle — if inflation is the dominant concern, CPI/PCE matter most. If recession risk is rising, GDP/PMI/labour lead.
Geopolitics overrides most of the above when active. War, trade wars, sanctions, embargos, these create uncertainty and fear that dominates flows regardless of economic data. Treat geopolitical events as regime-level inputs, not news events.

PILLAR 3: ECONOMIC EVENTS AND RISK SENTIMENT
News events create immediate bias. Analyse every high-impact event using the three data points: actual, expected, previous.
The key combinations and their expected behaviour:
- Above expected AND above previous (when expected is higher than previous): strong continuation move, some priced in already but energy carries to next significant level before retracing and continuing.
- Above expected BUT below previous: initial reversal move against expected and what is already priced in, then slowly reverses and follows trend due to being below previous.
- Below expected BUT above previous: initial shock reversal move against expectation due to expected, then slowly reverses and follows trend due to above previous.
- Below expected AND below previous (when expected is below previous): like the first example, continuation move, strong pressure.
- Above expected AND above previous (when expected is below previous): big surprise move, against current direction, breaking through significant levels, and possibly changing wider picture if its big enough news. 
...
PILLAR 4: INVESTOR PSYCHOLOGY AND CONNECTING THE DOTS
This is where the analysis becomes actionable. After working through Pillars 1-3, ask:
  - What is the dominant market narrative right now?
  - What are investors positioned for?
  - Where is the crowd wrong or early?
  - What would cause a surprise and how likely is it?
Front-run psychological movements: fear and greed move faster than economic logic. The market is not always rational but it is always telling you something. Read the behaviour, not just the data.
Explore alternative scenarios; if the base case is bearish USD, what would flip it? How probable is that scenario? This is not to hedge, it is to stay objective when the unexpected happens.
...
- Read risk sentiment proxies as a connected flow, not in isolation.
- Read the intermarket picture: if Gold drops and SP500 rises, risk is flowing on. If VIX drops alongside, confirm. If yields also rise, the move has rate-expectation backing. Build the narrative from the combination, not any single proxy.
- Market regime is also important, as it dictates what is the main perspective of the market - and allows for a quick understanding of what general consesus is and what could be a surprise. 
